...
def save_to_cache(video_id: str, concepts: list, timed_text: str, total_entries: int, duration: str) -> bool:
...
def get_global_daily_cost() -> float:
    try:
        conn = _get_connection()
        cursor = conn.cursor()
        cursor.execute('SELECT total_cost_usd FROM global_budget WHERE budget_date = DATE("now")')
        row = cursor.fetchone()
        conn.close()
