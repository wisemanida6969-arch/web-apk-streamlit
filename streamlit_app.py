...
st.warning("⚠️ GLOBAL VERSION (v3.0) - STABLE BUILD")
...
# ─────────────────────────────────────────────
st.markdown('<div style="background:rgba(26,26,33,0.6); padding:2.2rem; border-radius:20px; border:1px solid #32323D; text-align:center; margin-bottom:2rem;">', unsafe_allow_html=True)

if not st.session_state.user:
    if supabase:
        try:
            redirect_url = os.environ.get("REDIRECT_URL") or (st.secrets.get("REDIRECT_URL") if hasattr(st, "secrets") else "https://trytimeback.com")
            # Generate OAuth link on-demand to speed up initial load
            ...
            st.error(f"Sign-in failed to initialize: {e}")
    else:
        st.info("System initializing. Please refresh in a moment.")
else:
...
with t1:
    ...
    if st.button("Start Analysis →", use_container_width=True):
        if not url: st.error("Please enter a valid YouTube URL"); st.stop()
        if not st.session_state.user: st.error("Please sign in with Google to use the analyzer."); st.stop()
        
        vid_match = re.search(r"(?:v=|\/|be\/)([A-Za-z0-9_\-]{11})", url)
        vid = vid_match.group(1) if vid_match else None
        if not vid: st.error("Could not parse Video ID from the URL"); st.stop()
        
        # Initialize Analytics
        client = get_openai_client()
        if not client: st.error("AI Configuration Issue (Missing API Key)"); st.stop()
        
        with st.spinner("Analyzing transcript and extracting key concepts..."):
            try:
                ...
                res_gpt = client.chat.completions.create(
                    model="gpt-4o",
                    messages=[{"role": "system", "content": "Extract 3 core concepts in English. Format: JSON with 'concepts' array containing 'title', 'summary', and 'timestamp' string."}, {"role": "user", "content": text[:10000]}],
                ...
                try:
                    ...
                    st.warning(f"Note: Cache saving skipped ({db_e})")
    ...
    if st.session_state.analysis_results:
        ...
        with col_l:
            for i, c in enumerate(r.get("concepts", [])):
                st.markdown(f'<div style="background:#1A1A21; padding:1.5rem; border-radius:14px; border-left:5px solid #B19B72; margin-bottom:1rem; box-shadow: 0 4px 15px rgba(0,0,0,0.1);">'
                            f'<h3 style="color:#FFFFFF; margin-bottom:0.6rem;">{c.get("title", "Concept")}</h3>'
                            f'<p style="color:#E2E2E2; line-height:1.5;">{c.get("summary", "")}</p></div>', unsafe_allow_html=True)
        ...
        with col_r:
            ...
            st.markdown(f'<iframe width="100%" height="400" src="{embed}" frameborder="0" allow="autoplay; encrypted-media" allowfullscreen style="border-radius:16px;"></iframe>', unsafe_allow_html=True)

with t2:
    st.markdown("<center><h3 style='margin-bottom:1.5rem;'>Choose Your Development Plan</h3></center>", unsafe_allow_html=True)
    c1, c2, c3 = st.columns(3)
    with c1: st.info("**Basic / Free**\n\n$0 / month\n3 summaries per day")
    with c2: st.success("**Professional**\n\n$14.99 / month\nUnlimited summaries")
    with c3: st.warning("**Yearly Value**\n\n$99 / year\nBest Value")

st.markdown("<center style='color:#6E6E7A; padding:2rem; font-size:0.85rem;'>© 2026 YouTube Core Concept Analyzer • Global Stable v3.0</center>", unsafe_allow_html=True)
