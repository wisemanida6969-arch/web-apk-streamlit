...
#  Authentication UI
# ─────────────────────────────────────────────
st.markdown('<div style="background:rgba(26,26,33,0.5); padding:2rem; border-radius:16px; border:1px solid #32323D; text-align:center; margin-bottom:2.5rem;">', unsafe_allow_html=True)
...
with t1:
    ...
    if st.button("Start Analysis →", use_container_width=True):
        ...
        if not vid: st.error("Invalid format."); st.stop()
