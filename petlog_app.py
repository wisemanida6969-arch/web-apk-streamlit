...
def render_legal_terms_en():
    st.markdown("# 📜 Terms of Service")
    st.caption(f"Last updated: April 15, 2026")
    st.divider()
    st.markdown("""
**This service is operated by Trytimeback.**

## 1. Service Description and Effect of Terms

**PetLog AI** ("the Service") is a SaaS platform that helps users record
their pets' health status and provides monthly reports through photo
analysis powered by Claude AI. These Terms govern the rights, obligations,
and responsibilities between members and the operator regarding use of
the Service.

By accessing or using the Service, you are deemed to have agreed to these
Terms. The Service is available only to users aged 14 or older.

## 2. Membership and Accounts

- Accounts are created via Google OAuth. Only one PetLog account is
  permitted per Google account.
- Member information (email, name, profile image) must be accurate, and
  the member is responsible for all activity within the account.
- Transfer, lending, or sale of accounts is strictly prohibited.
- If you suspect your account security has been compromised, contact
  **admin@trytimeback.com** immediately.

## 3. Nature and Limitations of AI Photo Analysis

PetLog uses Claude AI Vision to analyze observable visual features in
pet photographs. **Please understand the following before using the
Service.**

- AI analysis results **do not replace a veterinarian's diagnosis.**
- Certain conditions cannot be detected from photos alone, and results
  may be inaccurate.
- If abnormalities are suspected or the AI returns a "warning" level,
  **please consult a veterinarian in person.**
- PetLog is not responsible for damages caused by delayed veterinary
  care resulting from sole reliance on AI analysis.

## 4. Paid Service (PetLog Pro)

- PetLog Pro is a recurring subscription priced at KRW 9,900/month
  (VAT excluded).
- Payments are processed securely through **Paddle**, whose terms of
  service also apply.
- Subscriptions may be cancelled at any time. Pro features remain
  available until the end of the current billing period.
- Refunds are governed by the separate **Refund Policy**.
- Prices may change from the next billing cycle after prior notice.

## 5. Prohibited Conduct

- Uploading or analyzing other people's photos without authorization
- Uploading photos that are not of pets (people, illegal material, etc.)
- Causing abnormal load on the Service or related APIs (Claude, Paddle)
- Sharing or selling account credentials, or abusing the Service via
  crawlers or automation tools

## 6. Suspension and Termination

- Members may request account deletion at any time by emailing
  **admin@trytimeback.com**.
- If a member violates these Terms or misuses the Service, the operator
  may restrict or terminate use without prior notice.
- Upon termination, data is handled according to the Privacy Policy.

## 7. Disclaimer

The Service is provided "as-is" without express or implied warranties.
PetLog is not liable for any direct or indirect damages arising from
service outages, data loss, or AI analysis errors.

## 8. Governing Law and Disputes

These Terms are governed by the laws of the Republic of Korea. Disputes
arising from the Service will be resolved in the courts of competent
...
def render_legal_privacy_en():
    st.markdown("# 🔒 Privacy Policy")
    st.caption(f"Last updated: April 15, 2026")
    st.divider()
    st.markdown("""
PetLog AI treats member personal information with care and complies with
the Korean Personal Information Protection Act and other applicable laws.

## 1. Information We Collect

**A. Google Account Information (at login)**
- Email address, name, profile image URL

**B. Pet Information (entered by member)**
- Name, species, breed, age, weight, emoji

**C. Health Records (entered by member)**
- Appetite, activity level, stool condition, weight, notes

**D. Uploaded Photos and AI Analysis Results**
- Pet photos uploaded by the member (stored as server files)
- Analysis text and scores generated by Claude AI

**E. Payment Information (Pro subscribers)**
- Paddle customer ID, subscription ID, transaction ID, subscription
  status, billing cycle information
- ※ **Sensitive payment data such as card numbers and CVC are stored
  only by Paddle.** PetLog never collects or retains this data.

## 2. Purpose of Use

- Providing the Service (health records, AI photo analysis, monthly reports)
- Member management and identity verification
- Payment processing and subscription management
- Enforcing per-plan usage limits
- Quality improvement and statistical analysis
- Responding to customer inquiries and fulfilling legal obligations

## 3. Third-Party Sharing and Processing

- **Google (OAuth)**: Login authentication.
  [Google Privacy Policy](https://policies.google.com/privacy)
- **Anthropic (Claude API)**: Uses uploaded photos for AI analysis.
  [Anthropic Privacy Policy](https://www.anthropic.com/privacy)
- **Paddle**: Payment processing and subscription management.
  [Paddle Privacy Policy](https://www.paddle.com/legal/privacy)
- **Railway**: Service hosting. Data is encrypted in transit and at rest.

## 4. Retention and Deletion

- When a member requests withdrawal, personal information is deleted
  without delay, and stored photo files are fully removed within 30 days.
- Data required by law will be retained only for the legally required
  period and then destroyed (e.g., 5 years for e-commerce transaction
  records).
- Deletion requests: **admin@trytimeback.com**

## 5. Member Rights

Members may request access, correction, deletion, or suspension of
processing of their personal information at any time, either via the
"My Info" section in the app or by email.

## 6. Cookies and Sessions

PetLog uses only the minimum session cookies required to maintain login
state. No advertising tracking cookies are used.

## 7. Data Protection Officer

- Contact: PetLog Operations
- Email: **admin@trytimeback.com**

For data protection complaints, you may also contact the Korea Internet
& Security Agency (KISA) Privacy Complaint Center
(privacy.kisa.or.kr, dial 118).
""")


def render_legal_refund_en():
    st.markdown("# 💰 Refund Policy")
    st.caption(f"Last updated: 2026-04-22")
    st.divider()
    st.markdown("""
We offer a 14-day refund policy in accordance with Paddle's terms of service.
...
Please send the following information to **admin@trytimeback.com**:
- Account email (Google login email)
- Date and amount of payment
- Paddle order number (shown in your payment confirmation email)
- Reason for the refund

Requests are processed **within 3-5 business days** of receipt.
Refunds are issued **to the original payment method** and may take an
additional **3-10 business days** depending on your card issuer or bank.

## 2. Subscription Cancellation

- Cancel directly from the Paddle customer portal
  (via the link in your payment confirmation email)
- Or email admin@trytimeback.com to request cancellation
- You may continue to use Pro features until the end of the current
  billing period
- Auto-billing stops from the next billing cycle
- Subscription cancellation and refunds are separate processes

## 3. Service Outage Refunds

If a service outage lasting **48 consecutive hours or more** occurs due
to fault on PetLog's side, you may request a prorated refund or service
extension.

## 4. Contact

Refund and payment inquiries: **admin@trytimeback.com**
""")


def _lang_switcher(doc: str, lang: str):
    other = "ko" if lang == "en" else "en"
    label = "한국어" if lang == "en" else "English"
    st.markdown(
        f'<div style="text-align:right;margin-bottom:6px;">'
        f'<a href="?page={doc}&lang={other}" target="_self" '
        f'style="color:#FF8FA3;text-decoration:underline;font-weight:600;">'
        f'🌐 {label}</a></div>',
        unsafe_allow_html=True,
    )


def render_legal_page(doc: str, lang: str = "ko"):
    c1, _ = st.columns([1, 5])
    with c1:
        back_label = "← Back" if lang == "en" else "← 뒤로"
        if st.button(back_label, key=f"back_legal_{doc}_{lang}"):
            st.query_params.clear()
            st.rerun()

    _lang_switcher(doc, lang)

    if doc == "terms":
        render_legal_terms_en() if lang == "en" else render_legal_terms()
    elif doc == "privacy":
        render_legal_privacy_en() if lang == "en" else render_legal_privacy()
    elif doc == "refund":
        render_legal_refund_en() if lang == "en" else render_legal_refund()
    else:
        st.error("잘못된 페이지입니다." if lang != "en" else "Invalid page.")
...
_legal_lang = st.query_params.get("lang", "ko")
if _legal_lang not in ("ko", "en"):
    _legal_lang = "ko"
if _legal_doc in ("terms", "privacy", "refund"):
    render_legal_page(_legal_doc, _legal_lang)
...
<div class="petlog-footer">
    ...
    <div style="margin-bottom:10px;font-size:0.85rem;">
        <a href="?page=terms&lang=en" target="_self"
           style="color:#9B7A7A;text-decoration:none;margin:0 8px;font-weight:500;">
           📜 Terms (EN)</a>
        ·
        <a href="?page=privacy&lang=en" target="_self"
           style="color:#9B7A7A;text-decoration:none;margin:0 8px;font-weight:500;">
           🔒 Privacy (EN)</a>
        ·
        <a href="?page=refund&lang=en" target="_self"
           style="color:#9B7A7A;text-decoration:none;margin:0 8px;font-weight:500;">
           💰 Refund (EN)</a>
    </div>
